...
- [`AWS_DEPLOYMENT_DEBUGGING.md`](AWS_DEPLOYMENT_DEBUGGING.md): post-deployment verification and failure diagnosis
- [`KAFKA_EXPLAINED.md`](KAFKA_EXPLAINED.md): Kafka itself, for the same audience
- [`ARCHITECTURE.md`](ARCHITECTURE.md) §3.2: the physical topology diagram
- [`SETUP.md`](SETUP.md) §5f: the full reasoning behind the MSK and ACL ordering
...
called **idempotence**, and this project depends on it.
...
Terraform forgets it owns your resources, which is normally a disaster. Here it
is safe, and §3 explains why.
...
few cannot change in place, so AWS destroys and recreates the resource. Terraform
...
flowchart TB
    ...
    subgraph L3["LAYER 3: Python and CLI, no Terraform"]
        direction TB
...
so it is durable and lockable, and something has to create that S3 bucket first.
...
resources nobody tracks, and a bill for them.
...
## 4. Layer 2: the platform (19 AWS resources and 4 local)
...
| `aws_secretsmanager_secret_policy` | Policy allowing `kafka.amazonaws.com` to read it | **The no-IAM-roles workaround.** A resource policy attaches to the secret and grants a service access. An IAM role would be a separate identity, and Stack A creates none |
...
applies. That matters because the configuration name is ForceNew, and a rename
would replace the whole Kafka cluster.
...
IAM roles, so the box has **no AWS identity at all.** That is why the git repo
must be public: the VM cannot authenticate to a private one.
...
duration. The file's presence is real evidence that the build finished, where a
`sleep 300` would be a guess.

### 4d. SSH key: 1 AWS resource and 3 local
...
Terraform generates it rather than reusing one, because the account is wiped
weekly and a hand-managed key is one more thing to recreate every cycle. It grants shell on a host that
...
4. Creating an ACL **requires an ACL** permitting it, namely `Alter` on the cluster. Enforce before granting, and you lock out even the tool that would fix it.
5. So ACLs must be granted **while still permissive**. ACLs can only be written over a **reachable broker**, and before public access the only reachable endpoint is the in-VPC one, port 9096.
6. The EC2 producer host is the only thing inside the VPC. **Hence the SSH rule.** It exists for exactly one step.

Steps 2 and 3 cannot merge into a single apply. Within one update the AWS
provider changes connectivity before configuration, which is the wrong order. It
would try public access first and fail exactly as it does with no configuration.
...
loosens both settings. It works in one pass because the provider does
connectivity before configuration: the wrong order for locking down, and the
right order for backing out.
...
|---|---|---|---|
| 1 | Layer 1 apply: S3 and DynamoDB | ~30 s | Terraform outputs the bucket name |
...
**Budget 45 to 60 minutes on the first run.** Trust that figure over the
per-step times, which vary and are indicative only. Steps 3,
...
success, so the script polls the API directly instead.
...
the finished state directly and skips steps 6 and 7, rather than walking back
through "permissive". That path would try to loosen ACL enforcement while public
access is still on, and AWS refuses it for the same reason it refuses the
reverse.
...
**Only `md.trades.v1` receives data.** The other 10 topics are provisioned and
empty, because no producer code exists for them yet. That is expected rather than
a failure. See [`ARCHITECTURE.md`](ARCHITECTURE.md) §5.
...
|---|---|---|
| Hangs at step 4, then times out at 900 s | `user_data` failed, usually at the git clone | `ssh ... 'sudo cat /var/log/cloud-init-output.log'` (the script prints this command for you) |
